...
Most mood-based music apps **match** your mood: sad person, sad playlist. That can deepen the feeling. HealthySongs is built on the opposite goal: **emotional regulation**, helping you toward a stable state.
...
HealthySongs uses the iso-principle carefully: the first track is close to how you feel but **always a step lighter**, never deeper into the feeling. Then each track moves further toward calm or brightness.
...
| Folder | What it is |
|---|---|
| [`healthysongs/`](healthysongs/) | **The phone app** (iPhone and Android), built with Expo / React Native. Face sensing, the regulation engine, journeys, Jen and Spotify. It has its own detailed [README](healthysongs/README.md). |
...
- [`blendshapes.ts`](healthysongs/src/engine/blendshapes.ts) turns those scores into two numbers, **valence** and **arousal**, using readable weights: smiling raises valence, frowning and brow-lowering lower it, wide eyes and an open jaw raise arousal. It's a heuristic on purpose, so you can read and argue with every weight.
- [`fusion.ts`](healthysongs/src/engine/fusion.ts) smooths the signal and compares it with your **resting face** (a slow moving average over about a minute), so it works across different faces.
...
[`regulate.ts`](healthysongs/src/engine/regulate.ts) holds the rules that keep the app on your side:
...
[`planner.ts`](healthysongs/src/engine/planner.ts) draws a path across the map from the "step lighter" start to the goal, in 2–8 steps. **Energy moves ahead of feeling**: when calming someone down, tempo drops first, then the music warms up. For each point on the path it picks the closest safe track that hasn't been played yet. If you drift away from the path, the remaining steps re-plan from where you are, still a step lighter.
...
|---|---|---|
| **Google MediaPipe Face Landmarker** (`@mediapipe/tasks-vision` 0.10.21, float16 model) | Reads 52 facial blendshapes per frame, on the phone. The model and WebAssembly are fetched from a CDN once (~8 MB), then cached. | [`faceWebViewHtml.ts`](healthysongs/src/sensing/faceWebViewHtml.ts) |
| **Jen music API** (Futureverse, `app.jenmusic.ai/api/v3/public`) | Text-to-music: `POST /track/generate` with a prompt and duration, then `GET /generation_status/:id` until the track is ready, then download it (Jen deletes files after 24 h). About $0.04 per track up to 60 s. | [`jen/client.ts`](healthysongs/src/jen/client.ts), [`jen/fill.ts`](healthysongs/src/jen/fill.ts) |
| **Spotify Web API** | Sign-in with Authorization Code + PKCE (no client secret). Endpoints: `GET /me/player/currently-playing`, `GET /me/player/queue`, `POST /me/player/queue`, `PUT /me/player/play`, `POST /me/player/next`, `GET /search` (limit 10), `GET /playlists/{id}/items`. Follows the February 2026 API changes. | [`spotify/`](healthysongs/src/spotify/) |
...
| **Expo / ngrok tunnel** | Lets the phone load the app from the laptop over the internet when Wi-Fi blocks direct connections. | [`metro.config.js`](healthysongs/metro.config.js) |

### Why Spotify songs aren't sorted by Spotify's own mood data

Spotify switched off `audio-features` (which included valence and energy) and `recommendations` for new apps in November 2024. So HealthySongs places Spotify songs using, in order of trust: **your tags**, **your zone playlists**, and **keyword search** (labelled as a guess), plus **what each song actually did to you**.
...
| Optional laptop servers | Python: FastAPI + MediaPipe ([`read_face.py`](healthysongs/server/read_face.py)), Jen proxy ([`jen_proxy.py`](healthysongs/server/jen_proxy.py)) |
...
HealthySongs was built with an AI coding assistant (Claude). Every contribution is logged in [`docs/ai-log.md`](docs/ai-log.md). In order:
...
2. Create `healthysongs/.env.local` (see [`healthysongs/.env.example`](healthysongs/.env.example)):
   ```
...
2. Add the **Redirect URI** that HealthySongs shows under **Settings → Spotify**. With the tunnel it looks like `https://<name>.exp.direct/spotify-relay`.
...
More detail is in [`healthysongs/README.md`](healthysongs/README.md) and [`HOW-TO-RUN.md`](HOW-TO-RUN.md).
...
- API keys live in `healthysongs/.env.local`, which is **never committed**. For anything shared, route Jen through the proxy in `healthysongs/server/jen_proxy.py` so the key stays on the laptop.
...
*HealthySongs is a 10-Day Challenge prototype, built with AI assistance and documented in the AI-use log.*
